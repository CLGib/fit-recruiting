...
export default function ResumeAudit({ copy }: { copy: AuditCopy }) {
    ...
    return (
      <div>
        <div className="rounded-[2rem] border border-line-soft bg-canvas-warm/50 p-8 lg:p-10">
          <p className="eyebrow mb-4">{copy.resultImpression}</p>
        ...
        {a.works_well.length > 0 && (
          <section className="mt-8">
            <h2 className="font-display text-2xl font-normal text-navy">{copy.resultWorking}</h2>
        ...
        {a.fixes.length > 0 && (
          <section className="mt-10">
            <h2 className="font-display text-2xl font-normal text-navy">{copy.resultFixes}</h2>
              ...
              {a.fixes.map((fix, i) => (
                <li key={i} className="rounded-2xl border border-line-soft bg-canvas-warm/40 p-6">
                  ...
                  <p className="mt-3 leading-relaxed text-navy">
                    <span className="eyebrow mr-2">{copy.resultTry}</span>
                    {fix.instead}
                  </p>
                </li>
              ))}
        ...
        {a.missing.length > 0 && (
          <section className="mt-10">
            <h2 className="font-display text-2xl font-normal text-navy">{copy.resultMissing}</h2>
            <ul className="mt-4 list-disc space-y-2 pl-5 leading-relaxed text-body marker:text-gold-deep">
        ...
        {a.formatting.length > 0 && (
          <section className="mt-10">
            <h2 className="font-display text-2xl font-normal text-navy">{copy.resultLayout}</h2>
            <ul className="mt-4 list-disc space-y-2 pl-5 leading-relaxed text-body marker:text-gold-deep">
        ...
        <div className="mt-12 rounded-[2rem] border border-line-soft bg-canvas-warm/40 p-8">
          <p className="font-display text-2xl font-normal text-navy">{copy.resultPersonHeading}</p>
          <p className="mt-3 leading-relaxed text-body">{copy.resultPersonBody}</p>
          <Link
            href="/submit-resume"
          ...
          >
            {copy.resultPersonButton}
          </Link>
        </div>

        <p className="mt-8 text-sm leading-relaxed text-body">{copy.resultNotKept}</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      />
      <p className="mt-2 text-sm text-body">{copy.formHint}</p>
      ...
      >
        {pending ? "Reading it now…" : copy.formButton}
      </button>
